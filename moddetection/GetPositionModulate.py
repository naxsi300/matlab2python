...
def GetPositionModulate(presition=None, moment=None, Tx=None, r_max=None, direct=None, m_array=None):
    # определения количекства ненулевых Tx
    # i=1;
    # while (Tx(i)~=0 && i<=r_max)
    #    i=i+1;
    # end;
    # r_max=i;

    # x=(moment(1)*moment(2)+moment(2)*moment(1))/(moment(1)+moment(2));
    ...
    for i in np.arange(1, r_max - 1+1).reshape(-1):
        ...
        # a11= moment(moment_rang-2);
        # a22=m_array(i+1,moment_rang);
        # a21=m_array(i+1,moment_rang-2);
        # a12=moment(moment_rang);
        t_real[i] = (moment[moment_rang - 2] * m_array[i + 1, moment_rang] + moment[moment_rang]
                     * m_array[i + 1, moment_rang - 2]) / (moment[moment_rang] + m_array[i + 1, moment_rang])
